...
import 'package:habit_tracker/detail_screen.dart';
...
class _AddHabitScreenState extends State<AddHabitScreen> {
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () async {
                if (_habitController.text.isNotEmpty) {
                  ...
                  // ✅ PERBAIKAN: SAVE HABITS SETELAH MENAMBAHKAN
                  await _saveHabits();

                  // Show snackbar untuk konfirmasi
            ...
            const SizedBox(height: 20),
            // ✅ PERBAIKAN: TAMBAHKAN HEADER UNTUK CLARITY
            Text(
              'All Habits (${allHabitsMap.length})',
              style: TextStyle(
                fontSize: 16,
                fontWeight: FontWeight.bold,
            ...
            Expanded(
              child: allHabitsMap.isEmpty
                  // ✅ PERBAIKAN: TAMPILKAN PESAN JIKA KOSONG
                  ? Center(
                      child: Text(
                  ...
                  : ListView(
                      children: allHabitsMap.entries.map((entry) {
                        ...
                        return ListTile(
                          ...
                          // NAVIGASI KE DETAIL SCREEN - Tap pada ListTile
                          onTap: () {
                            // Buat object item untuk dikirim ke DetailScreen
                            final item = {
                              'title': habitName,
                              'description':
                                  'This is your "$habitName" habit. '
                                  'Color: ${_getColorNameFromHex(entry.value)}. '
                                  'Keep up the good work!',
                              'color': habitColor,
                              'hexColor': entry.value,
                            };

                            // Navigate ke DetailScreen
                            Navigator.push(
                              context,
                              MaterialPageRoute(
  ...
  // Helper method untuk mendapatkan nama warna dari hex
  String _getColorNameFromHex(String hexColor) {
    Color color = _getColorFromHex(hexColor);
    String? colorName;

    _habitColors.forEach((name, habitColor) {
      if (habitColor.value == color.value) {
        colorName = name;
      }
    });
